...
class FeatureMaps:
    def __init__(self, basis_vectors: list, vector_dims: int, input_dim: int):
...
def laws_textures(vector_dims: int = 5, input_dim: int = 2):
    basis_vectors = [
        np.array([ 1, 2, 1]),
        np.array([-1, 0, 1]),
        np.array([-1, 2,-1])
    ]
    return FeatureMaps(basis_vectors, vector_dims, input_dim)


if __name__ == "__main__":
    laws_textures(vector_dims = 7)
